...
const items = JSON.parse(fs.readFileSync('scripts/raw-ian-10.json', 'utf8'));
...
async function run() {
  console.log(`Filtering and checking ${items.length} items...`);
  const valid = [];
  const invalid = [];
  ...
  fs.writeFileSync('scripts/valid-ian-10.json', JSON.stringify(valid, null, 2));
  console.log(`Saved scripts/valid-ian-10.json`);
}
