...
const items = JSON.parse(fs.readFileSync('scripts/raw-ian-12.json', 'utf8'));
...
async function run() {
  console.log(`Filtering and checking ${items.length} items for 2026-09-12...`);
  const valid = [];
  const invalid = [];
  ...
  fs.writeFileSync('scripts/valid-ian-12.json', JSON.stringify(valid, null, 2));
  console.log(`Saved scripts/valid-ian-12.json`);
}
